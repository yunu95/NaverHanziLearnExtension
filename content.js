...
const ICHING_HANZIS ="易, 太, 極, 陰, 陽, 卦, 爻, 彖, 象, 乾, 坤, 震, 巽, 坎, 離, 艮, 兌, 元, 亨, 利, 貞, 吉, 凶, 悔, 吝, 咎, 无, 厲, 屯, 蒙, 需, 訟, 師, 比, 小, 畜, 履, 泰, 否, 同, 人, 大, 有, 謙, 豫, 隨, 蠱, 臨, 觀, 噬, 嗑, 賁, 剝, 復, 妄, 頤, 過, 咸, 恒, 遁, 壯, 晉, 明, 夷, 家, 睽, 蹇, 解, 損, 益, 夬, 姤, 萃, 升, 困, 井, 革, 鼎, 漸, 歸, 妹, 豊, 旅, 渙, 節, 中, 孚, 旣, 濟, 未, 往, 來, 進, 退, 得, 失, 君, 子, 正,龍, 馬, 牛, 羊, 豚, 魚, 雉, 狐, 虎, 豹, 鼠, 鳥, 飛, 潛, 躍, 鳴, 金, 木, 土, 絲, 帛, 輿, 車, 舟, 甁, 樽, 桷, 庭, 門, 戶, 巷, 邑, 國, 公, 侯, 臣, 夫, 婦, 子, 孫, 兄, 弟, 師, 賓, 朋, 友, 寇, 盜, 聖, 賢, 庸, 妻, 妾, 祖, 考, 剛, 柔, 健, 順, 險, 陷, 麗, 說, 悅, 動, 靜, 壯, 弱, 盛, 衰, 盈, 虛, 滿, 窮, 通, 連, 斷, 固, 變, 化, 行, 昇, 降, 出, 入, 獲, 喪, 求, 予, 施, 受, 視, 聽, 言, 語, 默, 笑, 哭, 歌, 舞, 祭, 祀, 征, 伐, 戰, 遇, 涉, 待, 德, 仁, 義, 禮, 智, 信, 誠, 敬, 忠, 孝, 志, 慮, 幾, 神, 靈, 鬼, 命, 性, 心, 情, 道, 理, 法, 經, 權, 應, 比, 承, 乘"
const TAOTECHING_HANZIS ="道, 德, 玄, 牝, 樸, 嗇, 慈, 儉, 恬, 淡, 恍, 惚, 寥, 寂, 漠, 澹, 混, 沌, 皦, 昧, 渙, 儼, 窈, 冥, 虛, 實, 强, 弱, 剛, 柔, 巧, 拙, 辯, 訥, 盈, 沖, 爲, 爭, 伐, 矜, 彰, 跨, 企, 棄, 欲, 靜, 篤, 歸, 根, 覆, 命, 常, 明, 妄, 凶, 容, 公, 王, 天, 地, 久, 歿, 殆, 損, 益, 挫, 銳, 解, 紛, 和, 光, 同, 塵, 湛, 似, 象, 希, 夷, 微, 搏, 繩, 迎, 隨, 執, 御, 胞, 滌, 覽, 疵, 闔, 闢, 雌, 孺, 穀, 輻, 轂, 戶, 牖, 採, 贅, 疣, 寵, 辱, 驚"
const JANGZI_HANZIS = "逍, 遙, 遊, 齊, 忘, 齋, 化, 眞, 鯤, 鵬, 溟, 翼, 徙, 樗, 庖, 丁, 解, 刀, 骨, 節, 間, 寬, 游, 適, 安, 焉, 胡, 罔, 殆, 惑, 茫, 昧, 骸, 槁, 灰, 凝, 聽, 巵, 巹, 筌, 蹄, 罔, 象, 罔, 兩, 景, 辯, 訥, 齧, 缺, 寓, 巵, 曼, 衍"
const SUNTZU_HANZIS = "孫, 子, 兵, 法, 戰, 攻, 守, 謀, 詐, 詭, 勢, 節, 奇, 正, 虛, 實, 避, 擊, 迂, 直, 患, 利, 患, 專, 分, 形, 拙, 速, 智, 信, 仁, 勇, 嚴, 賞, 罰, 險, 隘, 廣, 遠, 近, 陷, 崩, 亂, 治, 靜, 譁, 佚, 飢, 飽, 勞, 逸, 蘇, 虜, 毁, 繼, 糧, 饋, 裹, 諜, 嚮, 內, 反, 鄕, 死, 生, 焚, 浸, 絶, 衝"
let hanzis = [];
...
const startDescriptionScroller = () => {
    ...
    const tryScroll = () => {
        ...
        } else if (prevTop !== null && Math.abs(top - prevTop) < 5) {
            // In view and position unchanged since last check — stable
            if (++stableCount >= 4) clearTimers();
        } else {
            // In view but position just shifted (page still loading content above)
            stableCount = 0;
            if (prevTop !== null) {
                element.scrollIntoView({ behavior: "smooth", block: "start" });
            }
...
const PANEL_ID = "hanzi-ext-panel";
...
const extractSoundMeaning = (currentHanzi) => {
    // Strategy 1: parse document title
    // Naver Hanja title format: "뜻 음 : 字 - 네이버 한자사전"  e.g. "불 화 : 火 - ..."
    const title = normalizeEntry(document.title);
    if (currentHanzi && title) {
        const escapedHanzi = currentHanzi.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const titleMatch = title.match(new RegExp(`^([^:]+)\\s*:\\s*${escapedHanzi}`));
        if (titleMatch) {
            const parts = titleMatch[1].trim().split(/\s+/);
            if (parts.length >= 2) {
                return { meaning: parts.slice(0, -1).join(" "), sound: parts[parts.length - 1] };
            }
            if (parts.length === 1) {
                return { meaning: "", sound: parts[0] };
            }
        }
    }
    ...
    const soundSelectors = [
        ".entry_title .sound", ".entry_head .sound", ".hanja_sound",
        ".read_sound", ".entry_sound", ".wordEntry .sound", "[class*='sound']",
    ];
    const meanSelectors = [
        ".entry_title .mean", ".hanja_mean", ".word_mean",
        ".entry_mean", ".mean_tit", ".mean_word", "[class*='mean']:not([class*='meaning'])",
    ];
    ...
    for (const sel of soundSelectors) {
        ...
        if (el) { sound = normalizeEntry(el.textContent); if (sound) break; }
    }
    for (const sel of meanSelectors) {
        const el = document.querySelector(sel);
        if (el) { meaning = normalizeEntry(el.textContent); if (meaning) break; }
    }
...
const renderPanel = () => {
    ...
    if (!panel) {
        ...
        panel.style.cssText = [
            ...
            "z-index:2147483647", "background:#fff3cd", "border:3px solid #d62828",
            "padding:14px 18px", "font-family:Arial,sans-serif", "color:#7a1f1f",
            "text-align:center", "min-width:110px", "border-radius:4px",
...
const startFloatingPanel = () => {
    ...
    // Re-render as page content loads dynamically, then stop after 6 s
    let debounce = null;
    panelMutationObserver = new MutationObserver(() => {
        clearTimeout(debounce);
        debounce = setTimeout(renderPanel, 200);
    });
    panelMutationObserver.observe(document.documentElement, { childList: true, subtree: true });
    setTimeout(() => {
...
const initPageLogic = async () => {
    ...
    historyUpdatedForCurrentPage = false;
    clearTimers();
    ...
    blurFocus();
    setTimeout(blurFocus, 100);
    setTimeout(blurFocus, 400);
    setTimeout(blurFocus, 1000);

...
const syncActivePreset = (presets) => {
    ...
    if (currentSig === getHanziSignature(parseHanziList(DEFAULT_HANZIS))) {
        activePresetId = "default";
        return;
    }
    if (currentSig === getHanziSignature(parseHanziList(ICHING_HANZIS))) {
        activePresetId = "iching";
        return;
    }
    if (currentSig === getHanziSignature(parseHanziList(TAOTECHING_HANZIS))) {
        activePresetId = "taoteching";
        return;
    }
    if (currentSig === getHanziSignature(parseHanziList(JANGZI_HANZIS))) {
        activePresetId = "jangzi";
        return;
    }
    if (currentSig === getHanziSignature(parseHanziList(SUNTZU_HANZIS))) {
...
const renderPresetBar = () => {
    ...
    loadPresets((presets) => {
        ...
        const ichingBtn = document.createElement("button");
        ichingBtn.className = "preset-btn preset-default" + (activePresetId === "iching" ? " preset-active" : "");
        ichingBtn.textContent = "주역";
        ichingBtn.title = "주역 한자 목록 불러오기";
        ichingBtn.addEventListener("click", () => {
            const hanzis = parseHanziList(ICHING_HANZIS);
            textarea.value = hanzis.join(", ");
            activePresetId = "iching";
        ...
        const taotechingBtn = document.createElement("button");
        taotechingBtn.className = "preset-btn preset-default" + (activePresetId === "taoteching" ? " preset-active" : "");
        taotechingBtn.textContent = "도덕경";
        taotechingBtn.title = "도덕경 한자 목록 불러오기";
        taotechingBtn.addEventListener("click", () => {
            const hanzis = parseHanziList(TAOTECHING_HANZIS);
            textarea.value = hanzis.join(", ");
            activePresetId = "taoteching";
        ...
        const jangziBtn = document.createElement("button");
        jangziBtn.className = "preset-btn preset-default" + (activePresetId === "jangzi" ? " preset-active" : "");
        jangziBtn.textContent = "장자";
        jangziBtn.title = "장자 한자 목록 불러오기";
        jangziBtn.addEventListener("click", () => {
            const hanzis = parseHanziList(JANGZI_HANZIS);
            textarea.value = hanzis.join(", ");
            activePresetId = "jangzi";
        ...
        const suntzuBtn = document.createElement("button");
        suntzuBtn.className = "preset-btn preset-default" + (activePresetId === "suntzu" ? " preset-active" : "");
        suntzuBtn.textContent = "손자병법";
        suntzuBtn.title = "손자병법 한자 목록 불러오기";
        suntzuBtn.addEventListener("click", () => {
            const hanzis = parseHanziList(SUNTZU_HANZIS);
            textarea.value = hanzis.join(", ");
            activePresetId = "suntzu";
...
const ICHING_HANZIS = "易, 太, 極, 陰, 陽, 卦, 爻, 彖, 象, 乾, 坤, 震, 巽, 坎, 離, 艮, 兌, 元, 亨, 利, 貞, 吉, 凶, 悔, 吝, 咎, 无, 厲, 屯, 蒙, 需, 訟, 師, 比, 小, 畜, 履, 泰, 否, 同, 人, 大, 有, 謙, 豫, 隨, 蠱, 臨, 觀, 噬, 嗑, 賁, 剝, 復, 妄, 頤, 過, 咸, 恒, 遁, 壯, 晉, 明, 夷, 家, 睽, 蹇, 解, 損, 益, 夬, 姤, 萃, 升, 困, 井, 革, 鼎, 漸, 歸, 妹, 豊, 旅, 渙, 節, 中, 孚, 旣, 濟, 未, 往, 來, 進, 退, 得, 失, 君, 子, 正, 龍, 馬, 牛, 羊, 豚, 魚, 雉, 狐, 虎, 豹, 鼠, 鳥, 飛, 潛, 躍, 鳴, 金, 木, 土, 絲, 帛, 輿, 車, 舟, 甁, 樽, 桷, 庭, 門, 戶, 巷, 邑, 國, 公, 侯, 臣, 夫, 婦, 孫, 兄, 弟, 賓, 朋, 友, 寇, 盜, 聖, 賢, 庸, 妻, 妾, 祖, 考, 剛, 柔, 健, 順, 險, 陷, 麗, 說, 悅, 動, 靜, 弱, 盛, 衰, 盈, 虛, 滿, 窮, 通, 連, 斷, 固, 變, 化, 行, 昇, 降, 出, 入, 獲, 喪, 求, 予, 施, 受, 視, 聽, 言, 語, 默, 笑, 哭, 歌, 舞, 祭, 祀, 征, 伐, 戰, 遇, 涉, 待, 德, 仁, 義, 禮, 智, 信, 誠, 敬, 忠, 孝, 志, 慮, 幾, 神, 靈, 鬼, 命, 性, 心, 情, 道, 理, 法, 經, 權, 應, 承, 乘";
...
const SUNTZU_HANZIS = "孫, 子, 兵, 法, 戰, 攻, 守, 謀, 詐, 詭, 勢, 節, 奇, 正, 虛, 實, 避, 擊, 迂, 直, 患, 利, 患, 專, 分, 形, 拙, 速, 智, 信, 仁, 勇, 嚴, 賞, 罰, 險, 隘, 廣, 遠, 近, 陷, 崩, 亂, 治, 靜, 譁, 佚, 飢, 飽, 勞, 逸, 蘇, 虜, 毁, 繼, 糧, 饋, 裹, 諜, 嚮, 內, 反, 鄕, 死, 生, 焚, 浸, 絶, 衝";
